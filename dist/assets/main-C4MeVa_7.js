(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))l(s);new MutationObserver(s=>{for(const n of s)if(n.type==="childList")for(const o of n.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&l(o)}).observe(document,{childList:!0,subtree:!0});function a(s){const n={};return s.integrity&&(n.integrity=s.integrity),s.referrerPolicy&&(n.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?n.credentials="include":s.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function l(s){if(s.ep)return;s.ep=!0;const n=a(s);fetch(s.href,n)}})();function Y(){return document.title="Ted Hebert | Find the Lever. | The Fractional Intelligence Company",_("description","Ted finds the hidden leverage point bleeding your operation dry — then builds the AI that fixes it in weeks. The Fractional Intelligence Company, powered by Razor Crest AI."),`
  ...
  `}function _(e,t){const a=document.querySelector(`meta[name="${e}"]`);a&&a.setAttribute("content",t)}function G(){return document.title="About | Ted Hebert | The Fractional Intelligence Company",`
  ...
  `}function Z(){return document.title="Services | Find. Build. Lift. | The Fractional Intelligence Company",`
  ...
  `}function W(){return document.title="Case Studies | The Fractional Intelligence Company",`
  ...
  `}const X="https://bqlxxgrgpipisqzmyhtz.supabase.co",P="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJxbHh4Z3JncGlwaXNxem15aHR6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjkxMDg4MzEsImV4cCI6MjA4NDY4NDgzMX0.vYwA8-VbAnyw62FGGd5t_ZZQ0c2tNSBnN8MSdwmPPrM",V=["All","Ghost Employee","Lever Insights","AI Operations","Industry"];let A="All",x=[],L=!1;function Q(){return document.title="Insights | The Fractional Intelligence Company",K(),`
    <section class="page-section page-section--cream" style="padding-top: calc(72px + var(--space-3xl));" aria-labelledby="insights-heading">
      <div class="container">
        ...
        <div class="insights-filter" id="insights-filter">
          ${V.map(e=>`
            <button class="filter-tag ${e==="All"?"active":""}" data-category="${e}">${e}</button>
          `).join("")}
        </div>
  ...
  `}async function K(){try{const e=await fetch(`${X}/rest/v1/articles?published=eq.true&order=published_at.desc`,{headers:{apikey:P,Authorization:`Bearer ${P}`}});if(!e.ok)throw new Error("Failed to fetch");x=await e.json(),L=!0,S(),ee()}catch{L=!0,x=[],S()}}function ee(){const e=document.getElementById("insights-filter");e&&e.addEventListener("click",t=>{const a=t.target.closest(".filter-tag");a&&(A=a.dataset.category,e.querySelectorAll(".filter-tag").forEach(l=>l.classList.remove("active")),a.classList.add("active"),S())})}function S(){const e=document.getElementById("insights-content");if(!e)return;const t=A==="All"?x:x.filter(i=>i.category===A);if(!L){e.innerHTML='<div class="insights-empty"><p>Loading...</p></div>';return}if(t.length===0){e.innerHTML=`
      <div class="insights-empty">
        <p class="insights-empty-title">No articles yet</p>
        <p>Check back soon. New insights are on the way.</p>
      </div>
    `;return}const a=t.find(i=>i.featured)||t[0],l=t.filter(i=>i.id!==a.id),s=i=>i?new Date(i).toLocaleDateString("en-US",{year:"numeric",month:"short",day:"numeric"}):"",n=i=>i==="Ghost Employee"||i==="AI Operations"?"pill--coral":i==="Lever Insights"?"pill--teal":"pill--gold";let o=`
    <div class="insights-featured" role="article">
      <p class="insights-featured-label">FEATURED</p>
      <h2 class="insights-featured-title">${g(a.title)}</h2>
      <p class="insights-featured-excerpt">${g(a.excerpt)}</p>
      <div class="insights-featured-meta">
        <span class="pill ${n(a.category)}">${g(a.category)}</span>
        <span class="insight-card-date">${s(a.published_at)}</span>
      </div>
    </div>
  `;l.length>0&&(o+='<div class="insights-grid">',l.forEach(i=>{o+=`
        <div class="insight-card" role="article">
          <h3 class="insight-card-title">${g(i.title)}</h3>
          <p class="insight-card-excerpt">${g(i.excerpt)}</p>
          <div class="insight-card-meta">
            <span class="pill ${n(i.category)}">${g(i.category)}</span>
            <span class="insight-card-date">${s(i.published_at)}</span>
          </div>
        </div>
      `}),o+="</div>"),e.innerHTML=o}function g(e){if(!e)return"";const t=document.createElement("div");return t.textContent=e,t.innerHTML}const te="https://bqlxxgrgpipisqzmyhtz.supabase.co",ae="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJxbHh4Z3JncGlwaXNxem15aHR6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjkxMDg4MzEsImV4cCI6MjA4NDY4NDgzMX0.vYwA8-VbAnyw62FGGd5t_ZZQ0c2tNSBnN8MSdwmPPrM";function se(){return document.title="Contact | Find Your Lever | The Fractional Intelligence Company",`
  ...
  `}function ie(){const e=document.getElementById("contact-form");e&&e.addEventListener("submit",async t=>{t.preventDefault();const a=document.getElementById("form-error"),l=document.getElementById("contact-submit"),s=document.getElementById("form-success");a.style.display="none";const n={name:e.name.value.trim(),email:e.email.value.trim(),company:e.company.value.trim(),role_title:e.role_title.value.trim()||null,message:e.message.value.trim()};if(!n.name||!n.email||!n.company||!n.message){a.textContent="Please fill in all required fields.",a.style.display="block";return}if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(n.email)){a.textContent="Please enter a valid email address.",a.style.display="block";return}l.disabled=!0,l.textContent="Sending...";try{if(!(await fetch(`${te}/functions/v1/contact-form`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${ae}`},body:JSON.stringify(n)})).ok)throw new Error("Submission failed");e.style.display="none",s.style.display="block"}catch{a.textContent="Something went wrong. Please email ted@raizorcrest.ai directly.",a.style.display="block",l.disabled=!1,l.textContent="Let's Find It →"}})}const ne="https://bqlxxgrgpipisqzmyhtz.supabase.co",oe="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJxbHh4Z3JncGlwaXNxem15aHR6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjkxMDg4MzEsImV4cCI6MjA4NDY4NDgzMX0.vYwA8-VbAnyw62FGGd5t_ZZQ0c2tNSBnN8MSdwmPPrM";function le(){return document.title="Free Download | Find the Lever — Digital Edition | The Fractional Intelligence Company",`
  ...
  `}function re(){const e=document.getElementById("download-form");e&&e.addEventListener("submit",async t=>{t.preventDefault();const a=document.getElementById("download-form-error"),l=document.getElementById("download-submit"),s=document.getElementById("download-btn-text"),n=document.getElementById("download-btn-loading"),o=document.getElementById("download-success");a.style.display="none";const i={name:e.name.value.trim(),company:e.company.value.trim(),title:e.title.value.trim(),email:e.email.value.trim(),siteUrl:`${window.location.protocol}//${window.location.host}`};if(!i.name||!i.company||!i.title||!i.email){a.textContent="All fields are required.",a.style.display="block";return}if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(i.email)){a.textContent="Please enter a valid email address.",a.style.display="block";return}l.disabled=!0,s.style.display="none",n.style.display="inline";try{const d=await fetch(`${ne}/functions/v1/lead-magnet`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${oe}`},body:JSON.stringify(i)}),c=await d.json();if(!d.ok)throw new Error(c.error||"Download failed");const u=c.downloadUrl,w=c.filename||"Find_the_Lever_-_Digital_Edition.pdf";e.style.display="none",o.style.display="flex";const T=document.getElementById("download-fallback-link");T&&(T.href=u,T.setAttribute("download",w));const y=document.createElement("a");y.href=u,y.download=w,document.body.appendChild(y),y.click(),document.body.removeChild(y)}catch(d){const c=d.message&&d.message.length<200?d.message:"Something went wrong. Please email ted@raizorcrest.ai directly.";a.textContent=c,a.style.display="block",l.disabled=!1,s.style.display="inline",n.style.display="none"}})}const de="https://bqlxxgrgpipisqzmyhtz.supabase.co",B="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJxbHh4Z3JncGlwaXNxem15aHR6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjkxMDg4MzEsImV4cCI6MjA4NDY4NDgzMX0.vYwA8-VbAnyw62FGGd5t_ZZQ0c2tNSBnN8MSdwmPPrM",F=`${de}/functions/v1/admin`,ce=["Ghost Employee","Lever Insights","AI Operations","Industry"];let M=null,v="contacts",p=null;function k(){return M||sessionStorage.getItem("admin_token")}function pe(e){M=e,sessionStorage.setItem("admin_token",e)}function z(){M=null,sessionStorage.removeItem("admin_token")}function r(e){if(!e)return"";const t=document.createElement("div");return t.textContent=e,t.innerHTML}function I(e){return e?new Date(e).toLocaleDateString("en-US",{year:"numeric",month:"short",day:"numeric"}):"—"}async function h(e,t={}){const a=k();return await fetch(`${F}/${e}`,{...t,headers:{"Content-Type":"application/json",Authorization:`Bearer ${B}`,"x-admin-token":a||"",...t.headers||{}}})}function ve(){return document.title="Admin | The Fractional Intelligence Company",k()?q():$()}function $(){return`
    ...
    <div class="admin-layout" style="padding-top: 72px; min-height: 100vh; background: var(--color-sand);">
      ...
      <div class="container" style="padding-bottom: var(--space-4xl);">
        <div class="admin-tabs" id="admin-tabs">
          <button class="admin-tab ${v==="contacts"?"active":""}" data-tab="contacts">Contacts</button>
          <button class="admin-tab ${v==="leads"?"active":""}" data-tab="leads">Lead Downloads</button>
          <button class="admin-tab ${v==="articles"?"active":""}" data-tab="articles">Articles</button>
          <button class="admin-tab ${v==="tools"?"active":""}" data-tab="tools">Tools</button>
        </div>
  ...
  `}function me(){if(!k()){N();return}U()}function N(){const e=document.getElementById("admin-login-form");e&&e.addEventListener("submit",async t=>{t.preventDefault();const a=document.getElementById("admin-password").value,l=document.getElementById("login-btn"),s=document.getElementById("login-error");l.textContent="Signing in...",l.disabled=!0,s.style.display="none";try{const n=await fetch(`${F}/login`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${B}`},body:JSON.stringify({password:a})}),o=await n.json();if(!n.ok){s.textContent=o.error||"Invalid password.",s.style.display="block",l.textContent="Sign In",l.disabled=!1;return}pe(o.token),v="contacts";const i=document.getElementById("main-content");i.innerHTML=q(),requestAnimationFrame(()=>U())}catch{s.textContent="Connection error. Please try again.",s.style.display="block",l.textContent="Sign In",l.disabled=!1}})}function U(){const e=document.getElementById("admin-logout-btn");e==null||e.addEventListener("click",()=>{z(),v="contacts";const a=document.getElementById("main-content");a.innerHTML=$(),requestAnimationFrame(()=>N())});const t=document.getElementById("admin-tabs");t==null||t.addEventListener("click",a=>{const l=a.target.closest(".admin-tab");l&&(v=l.dataset.tab,t.querySelectorAll(".admin-tab").forEach(s=>s.classList.remove("active")),l.classList.add("active"),O(v))}),O(v)}async function O(e){const t=document.getElementById("admin-tab-content");t&&(t.innerHTML='<div class="admin-loading">Loading...</div>',e==="contacts"?await ue(t):e==="leads"?await he(t):e==="articles"?await D(t):e==="tools"&&we(t))}async function ue(e){const t=await h("contacts");if(t.status===401){E();return}const{data:a,error:l}=await t.json();if(l||!a){e.innerHTML=`<div class="admin-empty">Failed to load contacts: ${r(l)}</div>`;return}if(a.length===0){e.innerHTML='<div class="admin-empty">No contact submissions yet.</div>';return}e.innerHTML=`
    <div class="admin-count">${a.length} submission${a.length!==1?"s":""}</div>
    <div class="admin-table-wrap">
      <table class="admin-table">
        ...
        <tbody>
          ${a.map(s=>`
            <tr>
              <td>${I(s.created_at)}</td>
              <td>${r(s.name)}</td>
              <td><a href="mailto:${r(s.email)}" style="color: var(--color-teal);">${r(s.email)}</a></td>
              <td>${r(s.company)||"—"}</td>
              <td>${r(s.role_title)||"—"}</td>
              <td class="admin-table-message">${r(s.message)}</td>
              <td>${s.email_sent?'<span class="pill pill--teal">Yes</span>':'<span class="pill pill--gold">No</span>'}</td>
            </tr>
          `).join("")}
        </tbody>
      </table>
    </div>
  `}async function he(e){const t=await h("leads");if(t.status===401){E();return}const{data:a,error:l}=await t.json();if(l||!a){e.innerHTML=`<div class="admin-empty">Failed to load leads: ${r(l)}</div>`;return}if(a.length===0){e.innerHTML='<div class="admin-empty">No lead magnet downloads yet.</div>';return}e.innerHTML=`
    <div class="admin-count">${a.length} download${a.length!==1?"s":""}</div>
    <div class="admin-table-wrap">
      <table class="admin-table">
        <thead>
          <tr>
        ...
        <tbody>
          ${a.map(s=>`
            <tr>
              ...
              <td>${r(s.company)||"—"}</td>
              <td>${r(s.title)||"—"}</td>
              <td>${s.downloaded_at?I(s.downloaded_at):'<span style="color:var(--color-coral);">Not yet</span>'}</td>
            </tr>
          `).join("")}
        </tbody>
      </table>
    </div>
  `}async function D(e){var n,o;const t=await h("articles");if(t.status===401){E();return}const{data:a,error:l}=await t.json();if(l){e.innerHTML=`<div class="admin-empty">Failed to load articles: ${r(l)}</div>`;return}const s=a||[];e.innerHTML=`
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: var(--space-lg);">
      <div class="admin-count">${s.length} article${s.length!==1?"s":""}</div>
      <button class="btn btn--coral btn--sm" id="new-article-btn">+ New Article</button>
    </div>
    <div id="article-form-container"></div>
    <div id="articles-list">
      ${s.length===0?'<div class="admin-empty">No articles yet. Create your first one above.</div>':`
        <div class="admin-table-wrap">
          <table class="admin-table">
            <thead>
              <tr>
  ...
  `,(n=document.getElementById("new-article-btn"))==null||n.addEventListener("click",()=>{p=null,H(null)}),(o=document.getElementById("articles-tbody"))==null||o.addEventListener("click",i=>{const m=i.target.closest("[data-edit-article]"),d=i.target.closest("[data-delete-article]");if(m){const c=m.dataset.editArticle,u=s.find(w=>String(w.id)===c);u&&H(u)}if(d){const c=d.dataset.deleteArticle;confirm("Delete this article? This cannot be undone.")&&fe(c)}})}function ge(e){return`
    <tr id="article-row-${e.id}">
      <td style="font-weight: 600;">${r(e.title)}</td>
      <td><span class="pill ${ye(e.category)}">${r(e.category)}</span></td>
      <td>${e.published?'<span class="pill pill--teal">Yes</span>':'<span class="pill pill--gold">Draft</span>'}</td>
      <td>${e.featured?'<span class="pill pill--coral">Yes</span>':"—"}</td>
      <td>${I(e.published_at||e.created_at)}</td>
      <td>
        <div style="display: flex; gap: var(--space-sm);">
          <button class="btn btn--sm btn--outline-teal" data-edit-article="${e.id}">Edit</button>
          <button class="btn btn--sm" style="border: 2px solid var(--color-coral); color: var(--color-coral); border-radius: var(--radius-pill); padding: 6px 16px; font-size: 0.875rem;" data-delete-article="${e.id}">Delete</button>
        </div>
      </td>
    </tr>
  `}function ye(e){return e==="Ghost Employee"||e==="AI Operations"?"pill--coral":e==="Lever Insights"?"pill--teal":"pill--gold"}function H(e){var a,l,s,n;p=e;const t=document.getElementById("article-form-container");t&&(t.innerHTML=`
    <div class="admin-form-card" id="article-form-wrap">
      <h3 style="font-size: 1.1rem; font-weight: 700; color: var(--color-navy); margin-bottom: var(--space-lg);">
        ${e?"Edit Article":"New Article"}
      </h3>
      <form id="article-form">
        <div class="admin-form-grid">
          <div class="form-group">
            <label class="form-label" for="art-title">Title *</label>
            <input class="form-input" type="text" id="art-title" value="${r((e==null?void 0:e.title)||"")}" required />
          </div>
          <div class="form-group">
            <label class="form-label" for="art-slug">Slug *</label>
            <input class="form-input" type="text" id="art-slug" value="${r((e==null?void 0:e.slug)||"")}" placeholder="url-friendly-slug" required />
          </div>
          <div class="form-group" style="grid-column: 1 / -1;">
            <label class="form-label" for="art-excerpt">Excerpt *</label>
            <textarea class="form-input form-textarea" id="art-excerpt" rows="3" required>${r((e==null?void 0:e.excerpt)||"")}</textarea>
          </div>
          <div class="form-group">
            <label class="form-label" for="art-category">Category *</label>
            <select class="form-input" id="art-category" required>
              ${ce.map(o=>`<option value="${o}" ${(e==null?void 0:e.category)===o?"selected":""}>${o}</option>`).join("")}
            </select>
          </div>
          <div class="form-group">
            <label class="form-label" for="art-date">Published Date</label>
            <input class="form-input" type="date" id="art-date" value="${e!=null&&e.published_at?e.published_at.substring(0,10):""}" />
          </div>
          <div class="form-group admin-checkboxes">
            <label class="admin-checkbox-label">
              <input type="checkbox" id="art-published" ${e!=null&&e.published?"checked":""} />
              <span>Published</span>
            </label>
            <label class="admin-checkbox-label">
              <input type="checkbox" id="art-featured" ${e!=null&&e.featured?"checked":""} />
              <span>Featured</span>
            </label>
          </div>
        </div>
        ...
        <div style="display: flex; gap: var(--space-md); margin-top: var(--space-xl);">
          <button type="submit" class="btn btn--coral" id="article-save-btn">${e?"Save Changes":"Create Article"}</button>
          <button type="button" class="btn btn--outline-teal" id="article-cancel-btn">Cancel</button>
        </div>
      </form>
    </div>
  `,t.scrollIntoView({behavior:"smooth",block:"nearest"}),(a=document.getElementById("art-title"))==null||a.addEventListener("input",o=>{if(!p){const i=document.getElementById("art-slug");i&&!i.dataset.touched&&(i.value=o.target.value.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/(^-|-$)/g,""))}}),(l=document.getElementById("art-slug"))==null||l.addEventListener("input",o=>{o.target.dataset.touched="true"}),(s=document.getElementById("article-cancel-btn"))==null||s.addEventListener("click",()=>{t.innerHTML="",p=null}),(n=document.getElementById("article-form"))==null||n.addEventListener("submit",async o=>{o.preventDefault(),await be()}))}async function be(){const e=document.getElementById("article-save-btn"),t=document.getElementById("article-form-error"),a=document.getElementById("article-form-success"),l={title:document.getElementById("art-title").value.trim(),slug:document.getElementById("art-slug").value.trim(),excerpt:document.getElementById("art-excerpt").value.trim(),category:document.getElementById("art-category").value,published:document.getElementById("art-published").checked,featured:document.getElementById("art-featured").checked,published_at:document.getElementById("art-date").value||null};p&&(l.id=p.id),e.textContent="Saving...",e.disabled=!0,t.style.display="none",a.style.display="none";try{const n=await h("articles",{method:p?"PUT":"POST",body:JSON.stringify(l)}),o=await n.json();if(!n.ok){t.textContent=o.error||"Failed to save article.",t.style.display="block",e.textContent=p?"Save Changes":"Create Article",e.disabled=!1;return}a.textContent=p?"Article updated!":"Article created!",a.style.display="block",e.textContent=p?"Save Changes":"Create Article",e.disabled=!1;const i=document.getElementById("admin-tab-content");i&&setTimeout(()=>D(i),800)}catch{t.textContent="Connection error. Please try again.",t.style.display="block",e.textContent=p?"Save Changes":"Create Article",e.disabled=!1}}async function fe(e){if((await h("articles",{method:"DELETE",body:JSON.stringify({id:e})})).status===401){E();return}const a=document.getElementById(`article-row-${e}`);a&&a.remove();const l=document.getElementById("articles-tbody");if(l&&l.children.length===0){const s=document.getElementById("admin-tab-content");s&&D(s)}}function we(e){var t,a,l;e.innerHTML=`
  ...
  `,(t=document.getElementById("seed-pdf-btn"))==null||t.addEventListener("click",async()=>{const s=document.getElementById("seed-pdf-btn"),n=document.getElementById("seed-pdf-msg");s.textContent="Seeding...",s.disabled=!0,n.style.display="none";try{const o=await h("seed-pdf",{method:"POST",body:JSON.stringify({siteUrl:window.location.origin})}),i=await o.json();n.style.display="block",o.ok?(n.className="form-success",n.textContent="PDF seeded successfully."):(n.className="form-error",n.textContent=i.error||"Failed to seed PDF.")}catch{n.style.display="block",n.className="form-error",n.textContent="Connection error."}s.textContent="Seed PDF",s.disabled=!1}),(a=document.getElementById("upload-pdf-btn"))==null||a.addEventListener("click",async()=>{const s=document.getElementById("pdf-file-input"),n=document.getElementById("upload-pdf-btn"),o=document.getElementById("upload-pdf-msg");if(!s.files[0]){o.style.display="block",o.className="form-error",o.textContent="Please select a PDF file first.";return}n.textContent="Uploading...",n.disabled=!0,o.style.display="none";try{const i=s.files[0],m=k(),d=new FormData;d.append("file",i,i.name);const c=await fetch(`${F}/upload-pdf`,{method:"POST",headers:{Authorization:`Bearer ${B}`,"x-admin-token":m||""},body:d}),u=await c.json();o.style.display="block",c.ok?(o.className="form-success",o.textContent="PDF uploaded successfully.",s.value=""):(o.className="form-error",o.textContent=u.error||"Upload failed.")}catch(i){o.style.display="block",o.className="form-error",o.textContent=i.message||"Connection error."}n.textContent="Upload PDF",n.disabled=!1}),(l=document.getElementById("change-password-btn"))==null||l.addEventListener("click",async()=>{const s=document.getElementById("new-password-input").value,n=document.getElementById("confirm-password-input").value,o=document.getElementById("change-password-btn"),i=document.getElementById("password-msg");if(i.style.display="none",!s||s.length<8){i.style.display="block",i.className="form-error",i.textContent="Password must be at least 8 characters.";return}if(s!==n){i.style.display="block",i.className="form-error",i.textContent="Passwords do not match.";return}o.textContent="Saving...",o.disabled=!0;try{const m=await h("change-password",{method:"POST",body:JSON.stringify({newPassword:s})}),d=await m.json();i.style.display="block",m.ok?(i.className="form-success",i.textContent="Password changed successfully.",document.getElementById("new-password-input").value="",document.getElementById("confirm-password-input").value=""):(i.className="form-error",i.textContent=d.error||"Failed to change password.")}catch{i.style.display="block",i.className="form-error",i.textContent="Connection error."}o.textContent="Change Password",o.disabled=!1})}function E(){z();const e=document.getElementById("main-content");e&&(e.innerHTML=$(),requestAnimationFrame(()=>N()))}const xe={"/":Y,"/about":G,"/services":Z,"/case-studies":W,"/insights":Q,"/contact":se,"/download":le,"/admin":ve},J=document.getElementById("main-content");function C(e){const t=xe[e];if(!t){C("/");return}J.innerHTML=t(),window.scrollTo(0,0),Ie(e),j(),requestAnimationFrame(()=>{document.body.classList.add("loaded"),Ee(),Ce(),e==="/contact"&&ie(),e==="/download"&&re(),e==="/admin"&&me()})}function Ie(e){document.querySelectorAll(".nav-link").forEach(t=>{const a=t.getAttribute("href");t.classList.toggle("active",a===e)})}document.addEventListener("click",e=>{const t=e.target.closest("[data-link]");if(!t)return;e.preventDefault();const a=t.getAttribute("href");a&&a!==window.location.pathname?(history.pushState(null,"",a),C(a)):a===window.location.pathname&&window.scrollTo({top:0,behavior:"smooth"})});window.addEventListener("popstate",()=>{C(window.location.pathname)});const f=document.getElementById("nav-hamburger"),R=document.getElementById("mobile-overlay");function j(){f.classList.remove("open"),f.setAttribute("aria-expanded","false"),R.classList.remove("open"),document.body.style.overflow=""}f.addEventListener("click",()=>{const e=f.classList.toggle("open");f.setAttribute("aria-expanded",String(e)),R.classList.toggle("open",e),document.body.style.overflow=e?"hidden":""});document.addEventListener("keydown",e=>{e.key==="Escape"&&R.classList.contains("open")&&j()});const ke=document.getElementById("nav");window.addEventListener("scroll",()=>{ke.classList.toggle("nav--scrolled",window.scrollY>10)},{passive:!0});let b;function Ee(){b&&b.disconnect();const e=J.querySelectorAll(".animate-on-scroll, .stagger-children, .tag-animate");b=new IntersectionObserver(t=>{t.forEach(a=>{a.isIntersecting&&(a.target.classList.add("visible"),b.unobserve(a.target))})},{threshold:.15,rootMargin:"0px 0px -40px 0px"}),e.forEach(t=>b.observe(t))}function Ce(){const e=document.getElementById("scroll-indicator");if(!e)return;const t=()=>{e.classList.toggle("faded",window.scrollY>100)};window.addEventListener("scroll",t,{passive:!0})}C(window.location.pathname);
